...
-   For whom? Who will use it and what is their background?
    -   The teacher dashboard is for the course instructor or teaching team, who
    already understand the course structure, quizzes, and clicker questions but 
    need a fast way to monitor patterns across the whole class.
    -   This user is less interested in one specific student and more interested 
    in identifying overall trends, participation issues, and places where 
    intervention may be needed.

-   Why? What is the goal? What questions to answer?
    -   The goal is to help instructors identify trends across sessions, 
    evaluate how engagement (clicker completion) relates to performance, and 
    understand the incoming skill profile of their class.
    -   The dashboard should answer questions like: How is attendance changing 
    over time? Which sessions had weaker quiz performance? Is clicker engagement 
    related to quiz success? What kind of skill background does the class enter 
    with?

-   What? What data to show and what is its structure?
    -   The dashboard should show class-level summaries from the quiz dataset, 
    including attendance, average quiz scores, clicker completion, and 
    session-by-session trends.
    -   It also incorporates aggregated student data to examine relationships 
    between engagement and performance (e.g., clicker completion vs quiz 
    scores), along with skill-survey summaries from the experience dataset to 
    provide background context about the class.

-   How? How will visualizations support the goal?
    -   Info boxes give the instructor an immediate snapshot of class size, 
    attendance, quiz performance, and clicker participation without requiring 
    interpretation of a full graph.
    -   Trend charts across sessions help the instructor see when performance or
    participation changes over time, while the scatterplot and skill chart help 
    explain relationships between engagement and outcomes and provide context 
    about the class’s incoming preparation.
...
![Student Dashboard Screenshot](student_dashboard_screenshot.png)

![Instructor Dashboard Screenshot](instructor_dashboard_screenshot.png)
...
-   What do you like about it?
    -   We like that the information (such as students, attendance, quiz score) are on top because it makes it easier for the professor to see a snapshot of the class. It gives a good summary without needing to interpret any of the graphs. We also really like the scatterplot of clicker engagement vs. quiz performance, since it confirms that students who complete more clicker questions tend to score higher, which is a useful and actionable insight for the instructor.
-   What would you change or add to it if you had more time?
    -   If we have more time, we should add a color-coded threshold line on the quiz score trend chart, as in marking scores below a certain value in red, so the instructor can easily spot underperforming sessions without having to mentally interpret the y-axis scale. We would also add hover labels or tooltips so it is easier to read exact values.
-   What was the biggest challenge you faced? How did you address it?
    -   The biggest challenge was aggregating the data at the right level because some visualizations needed session-level summaries while others needed student-level summaries, and the year filter had to flow through all of them consistently. We addressed this by pre-computing separate summary tables (class_by_session, inst_student_summary, class_skills) during data preprocessing, then filtering them based on the selected year.
...
-   How much did you use generative AI (e.g., not at all, some, most, or all the questions) and which one did you use? We did not use generative AI.
-   If you used generative AI, how did you use it and was it helpful? N/A
